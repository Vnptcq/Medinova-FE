...
export default function SurgeryPage() {
  ...
  useEffect(() => {
    // Load consultations - will use getAllSurgeryConsultations and filter by doctorId if available
  ...
  const loadSurgeryConsultations = async () => {
    try {
      ...
      // Use getMyDoctorConsultations endpoint (for current doctor)
      // This endpoint returns consultations assigned to the current doctor
      ...
      // Call the new my-doctor-consultations endpoint
      const apiResponse = await axios.get(`${baseURL}/api/surgery-consultations/my-doctor-consultations`, {
      ...
      let consultations = Array.isArray(apiResponse.data) ? apiResponse.data : [];
      
      // If no consultations from my-doctor-consultations, try fallback
      if (consultations.length === 0 && doctorId) {
        console.log("No consultations from my-doctor-consultations, trying fallback with doctorId:", doctorId);
        const surgeryApi = getSurgeryConsultationManagement();
        try {
          const fallbackResponse = await surgeryApi.getSurgeryConsultationsByDoctor(
            doctorId,
            statusFilter ? { status: statusFilter } : undefined
          );
          consultations = Array.isArray((fallbackResponse as any)?.data) 
      ...
      if (error?.response?.status === 403) {
        console.error("Access denied. User may not be a doctor or endpoint not available.");
      }
